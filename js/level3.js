...
$("#show").click(function(e) {
    e.preventDefault();
    var year = $("#showByYear").val()
    var countries = $("#showByCountry").val()
    if (year !== "" && countries.length !== 0) {
      $(".instructions").addClass("hidden")
      $("#instructionsInfo").removeClass("hidden")
    } else {
      $(".instructions").addClass("hidden")
      $("#instructionsWarning").removeClass("hidden")
    }
    $(".parcoords > *").remove();
    $("#chart-line1 > *").remove();
    $("#timeSeriesTitle").html("");
    d3.csv('/data/WorldEconomy1990-2015.csv', function(data) {
      pc1 = d3.parcoords()("#level3")
        .data(data.filter(function(d){
            if (d['Year'] == year && $.inArray(d['Region'],countries) >= 0) {
              return d
            }}))
        .hideAxis(["Year"])
        .composite("darken")
        .color(function(d) { return rainbow(d['Region']); })
        .alpha(0.35)
        .margin({ top: 40, left: 120, bottom: 12, right: 0 })
        .render()
        .reorderable()
        .brushMode("1D-axes")  // enable brushing
        .interactive()  // command line mode

      pc1.svg.selectAll(".label")
    ...
    $(document).ready(function(){
      $(".label").click(function(){
        var countries = $("#showByCountry").val()
        var selectedAxis = $(this).text()
        if (selectedAxis !== "Region Name" && selectedAxis !== "Region") {
          $(".instructions").addClass("hidden")
          $("#instructionsSuccess").removeClass("hidden")
          var path = (function() { return ("/data/" + selectedAxis + ".csv")})();
          $('#timeSeriesTitle').text(selectedAxis + " 1990-2015")
          $('#chart-line1').html("")
          d3.csv(path, function(error, data) {
            data.forEach(function (d) {
              d.year = +d.Year;
              d.variableA = +d['Eastern Africa'];
              d.variableB = +d['Middle Africa'];
              d.variableC = +d['Northern Africa'];
              d.variableD = +d['Southern Africa'];
              d.variableE = +d['Western Africa'];
              d.variableF = +d['Eastern Asia'];
              d.variableG = +d['Central Asia'];
              d.variableH = +d['Southern Asia'];
              d.variableI = +d['South-Eastern Asia'];
              d.variableJ = +d['Western Asia'];
              d.variableK = +d['Eastern Europe'];
              d.variableL = +d['Northern Europe'];
              d.variableM = +d['Southern Europe'];
              d.variableN = +d['Western Europe'];
              d.variableO = +d['Northern America'];
              d.variableP = +d['Caribbean'];
              d.variableQ = +d['Central America'];
              d.variableR = +d['South America'];
              d.variableS = +d['Oceania'];
            });
            var linesToDisplay = {}
            if ($.inArray("1",countries) >= 0) {linesToDisplay['Eastern Africa'] = {column: 'variableA'}}
            if ($.inArray("2",countries) >= 0) {linesToDisplay['Middle Africa'] = {column: 'variableB'}}
            if ($.inArray("3",countries) >= 0) {linesToDisplay['Northern Africa'] = {column: 'variableC'}}
            if ($.inArray("4",countries) >= 0) {linesToDisplay['Southern Africa'] = {column: 'variableD'}}
            if ($.inArray("5",countries) >= 0) {linesToDisplay['Western Africa'] = {column: 'variableE'}}
            if ($.inArray("7",countries) >= 0) {linesToDisplay['Eastern Asia'] = {column: 'variableF'}}
            if ($.inArray("8",countries) >= 0) {linesToDisplay['Central Asia'] = {column: 'variableG'}}
            if ($.inArray("9",countries) >= 0) {linesToDisplay['Southern Asia'] = {column: 'variableH'}}
            if ($.inArray("10",countries) >= 0) {linesToDisplay['South-Eastern Asia'] = {column: 'variableI'}}
            if ($.inArray("11",countries) >= 0) {linesToDisplay['Western Asia'] = {column: 'variableJ'}}
            if ($.inArray("13",countries) >= 0) {linesToDisplay['Eastern Europe'] = {column: 'variableK'}}
            if ($.inArray("14",countries) >= 0) {linesToDisplay['Northern Europe'] = {column: 'variableL'}}
            if ($.inArray("15",countries) >= 0) {linesToDisplay['Southern Europe'] = {column: 'variableM'}}
            if ($.inArray("16",countries) >= 0) {linesToDisplay['Western Europe'] = {column: 'variableN'}}
            if ($.inArray("18",countries) >= 0) {linesToDisplay['Northern America'] = {column: 'variableO'}}
            if ($.inArray("20",countries) >= 0) {linesToDisplay['Caribbean'] = {column: 'variableP'}}
            if ($.inArray("21",countries) >= 0) {linesToDisplay['Central America'] = {column: 'variableQ'}}
            if ($.inArray("22",countries) >= 0) {linesToDisplay['South America'] = {column: 'variableR'}}
            if ($.inArray("24",countries) >= 0) {linesToDisplay['Oceania'] = {column: 'variableS'}}
            var chart = makeLineChart(data, 'year', linesToDisplay, {xAxis: 'Years', yAxis: ''});
            chart.bind("#chart-line1");
            chart.render();
          });
        } else {
          $(".instructions").addClass("hidden")
          $("#instructionsDanger").removeClass("hidden")
        }
      })
    })
});
